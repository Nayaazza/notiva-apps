    ...
    tasks.forEach((task, index) => {
        ...
        <div class="task-item">
            <div>
                ...
                <small style="opacity:0.7;">📅 ${task.date || "Tidak ditentukan"}</small>
            </div>
...
function renderMiniCalendar() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    div.onclick = () => {
      document.querySelectorAll(".mini-day").forEach(el => el.classList.remove("active"));
...
function loadStatisticsData() {
  ...
  document.getElementById("progressStatus").innerText = `${completed} / 7 Completed`;

  document.querySelector(".progress-circle").style.borderColor = percent >= 100 ? "var(--success)" : "var(--primary)";
...
function loadProfile() {
    ...
    if (profile) {
        ...
        document.getElementById("profileGoal").innerText = 
            profile.goal ? `🎯 Target Harian: ${profile.goal} task` : "";
...
function saveProfile(){
    const profile = {
        ...
        photo: localStorage.getItem("tempPhoto") || document.getElementById("profilePic").src
    };
